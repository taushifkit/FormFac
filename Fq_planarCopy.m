...
F1mv2l = ( C2 + (C1*G1*LQm)/2 + (G2*LQm)/2 - (A2*LQm^2)/4 - (A1*C1*LQm^2)/4 - 
 ...
 ep^3*(-(C1*G1*LQm^4)/48 - (G2*LQm^4)/6 + (A2*LQm^5)/30 + (A1*C1*LQm^5)/240 + 
   ...
   (A1^2*LQm^7)/320) );
F1mv2lres = Collect[Expand[Normal[Series[F1mv2l /. subs1 /. subs2 /. subs3 /. subs4, {ep, 0, 3}]]], {ep, nc, nl, nfv}];
...
F1mv3lres = Collect[Expand[Normal[Series[F1mv3l /. subs1 /. subs2 /. subs3 /. subs4, {ep, 0, 2}]]], {ep, nc, nl, nfv}];
...
lnFmv = (asm*lnF1mv1l + asm^2*lnF1mv2l + 0 asm^3*lnF1mv3l + 0 asm^4*lnF1mv4l);
lnFmv = lnFmv /. {asm -> au*mBYmu*ZasmI};
lnFmv = Normal[Series[lnFmv /. {ZasmI -> (1 + au zasmI1 + au^2 zasmI2 + au^3 zasmI3 + au^4 zasmI4)}, {au, 0, 4}]];
lnFmv = Normal[Series[Expand[lnFmv /. subs5 /. subs6], {ep, 0, 4}]];
